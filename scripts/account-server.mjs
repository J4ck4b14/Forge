...
const dataFile = resolve(
  process.env.FORGE_ACCOUNT_DATA || '.forge/accounts.json',
);
const sessions = new Map();
const authAttempts = new Map();
const sessionTtlMs = Math.max(
  60_000,
  Number(process.env.FORGE_SESSION_TTL_MS || 7 * 24 * 60 * 60 * 1000),
);
const authWindowMs = Math.max(
  60_000,
  Number(process.env.FORGE_AUTH_WINDOW_MS || 15 * 60 * 1000),
);
const authLimit = Math.max(1, Number(process.env.FORGE_AUTH_LIMIT || 20));
const maxProjectsPerUser = Math.max(
  1,
  Number(process.env.FORGE_MAX_PROJECTS || 100),
);
const maxProjectBytes = Math.max(
  1024,
  Number(process.env.FORGE_MAX_PROJECT_BYTES || 20 * 1024 * 1024),
);
