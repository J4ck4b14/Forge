...
try {
  ...
  model.load(
    JSON.parse(
      await readFile(
        'examples/physics-playground/Playground.forge.json',
        'utf8',
      ),
    ),
  );
  ...
  model.change('Example media', () => {
    ...
    model.world.add(player.id, 'forge.animator', { controller, speed: 1 });
    model.world.add(player.id, 'forge.audio-source', {
      clip: audio,
      volume: 0.7,
      loop: false,
      rate: 1,
    ...
    model.world.set(player.id, 'forge.sprite', {
      ...model.world.components(player.id).get('forge.sprite'),
      texture: images[0],
    });
  });
