...
try {
  ...
  const entity = (
    ...
    if (width)
      m.world.add(id, 'forge.sprite', {
        ...m.registry.get('forge.sprite').defaults(),
        width,
        height,
        tint,
        texture,
  ...
  const light = (name, kind, mobility, x, y, values = {}) => {
    ...
    component(id, 'forge.light', { kind, mobility, ...values });
    return id;
  };
  const platform = (name, x, y, width, visible = true) => {
    ...
    component(id, 'forge.box-collider', { width, height: 24 });
    component(id, 'forge.rigidbody', { mode: 'static' });
    return id;
  };
  const wall = (name, x, y, width, height, tint = '#526969') => {
    ...
    component(id, 'forge.box-collider', { width, height });
    component(id, 'forge.rigidbody', { mode: 'static' });
    component(id, 'forge.shadow-caster', { width, height });
    return id;
  };
  const decor = (
    name,
  ...
  const emitter = (name, values = {}) => {
    ...
    component(id, 'forge.particle-emitter', {
      emitting: false,
      playOnAwake: false,
      rate: 0,
      burst: 0,
  ...
  const sound = (name, clip, volume) => {
    const id = entity(name, 0, 0);
    component(id, 'forge.audio-source', {
  ...
  const uiText = (
    ...
    component(id, 'forge.ui-panel', { color: '#05090e', opacity: 0.82 });
    component(id, 'forge.ui-layout', {
      anchor,
      width,
      padding: 9,
      margin: 12,
  ...
  const baseRoom = (room) => {
    ...
    component(camera, 'forge.camera', { background: '#000000', zoom: 1 });
    ...
    component(player, 'forge.rigidbody', { continuous: true });
    component(player, 'forge.box-collider', {
      width: 27,
      height: 37,
      friction: 0,
    });
    component(player, 'forge.animator', { controller });
    component(player, 'forge.audio-source', {
      clip: assetIds['Audio/Action.wav'],
      volume: 0.23,
      bus: 'SFX',
    });
    const behaviour = randomUUID();
    component(player, 'forge.behaviours', {
      order: [behaviour],
      items: {
        [behaviour]: {
          id: behaviour,
  ...
  await writeFile(`${root}/showcase.forge.json`, json);
  await mkdir('public/examples', { recursive: true });
  await writeFile('public/examples/showcase.forge.json', json);
  console.log(
