...
**Milestones 0–7 and Forge 0.9–0.16.1 are implemented.** Forge 0.16.1 adds deterministic Godot 4 project generation to the Interchange foundation: scenes and hierarchy, transforms, copied media, sprites, physics shapes/bodies, cameras, radial 2D lights, spatial/non-spatial audio, input actions, audio buses, portable Behaviour Graph execution, stable IDs, manifests, and explicit fidelity diagnostics.

## Three prototype workshop

Read [Forge-Prototype-Workshop.docx](docs/Forge-Prototype-Workshop.docx) or its [Markdown source](docs/workshop.md). The 19-page guide explains reconstruction from a blank scene, exact entities/settings, scripts, tests, common mistakes and export.
...
Extract the archive and open a terminal in the directory containing **package.json and package-lock.json**. This archive puts those files at its root; there is no extra `forge` folder inside it.
...
```text
examples/milestones-5-7/Workshop.forge.json
```
...
- **Project continuity:** Account is optional. The reference Node server has revision conflict protection plus expiring/revocable sessions, bounded auth attempts and storage quotas. Local IndexedDB and portable `.forge.json` export/import remain independently available.
...
See [Forge 0.9 Editor Quality](docs/editor-quality-0.9.md) for the implementation contracts.

## Public alpha: zero-cost hosting

Forge can be hosted as a static site. **Save locally** writes projects to IndexedDB in the current browser; **Export backup** downloads a portable `.forge.json`; **Import project** opens that file later or on another device. Accounts are therefore optional for the public alpha. A first-run notice explains the storage model instead of silently implying that browser storage is a cloud backup.

For a free launch, push the repository to GitHub and enable **Settings → Pages → GitHub Actions**. The checked-in Pages workflow verifies Forge, builds `dist/`, and deploys it. Vite uses relative production paths, so a project URL such as `https://YOUR_USERNAME.github.io/Forge/` works without a custom domain. See [zero-cost GitHub Pages deployment](docs/github-pages.md).
