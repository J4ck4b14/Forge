# Forge 0.9.3 public-alpha launch pass

Forge 0.9.3 turns the certified 0.9.2 editor into a zero-cost static-hosting release without changing project schema v4.
...
- **Export backup** downloads a portable `.forge.json` project.
- **Import project** accepts `.forge.json`/JSON and validates it before replacing the current editor state.
...
This design makes accounts optional: GitHub Pages can host the complete editor while authors move projects between devices manually with `.forge.json` files.

## Static deployment

- Vite uses a relative production base so builds work below a repository path such as `/Forge/`.
