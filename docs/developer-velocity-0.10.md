# Forge 0.10 Developer Velocity

Forge 0.10 removes the single-script attachment limit and makes common runtime coordination available through shared engine services.

## Behaviour composition

`forge.behaviours` contains an authored order plus identity-keyed behaviour records. Each record owns its enabled state, TypeScript asset and exposed values. IDs remain stable through save/load and prefab propagation; runtime ownership combines the entity GUID with the behaviour ID. Lifecycle teardown clears listeners, timers and tweens even when a script is replaced or its entity is destroyed.
