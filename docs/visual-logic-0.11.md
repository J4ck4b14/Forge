...
Forge Behaviour Graphs are versioned JSON assets (`application/x-forge-behaviour-graph`). Graph, node, connection and group identities are stable; graph variables and node properties remain authored data rather than generated source.

## Runtime architecture

Graph and TypeScript Behaviours occupy the same ordered `forge.behaviours` component and run through `ScriptSystem`. Both receive the same `ScriptContext`, including input, physics, animation, audio, signals, timers, tweens, prefabs, coordinates and scene loading. Owner-scoped cleanup therefore applies equally to both forms.
