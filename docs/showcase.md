...
The Luminous Vault is Forge's complete vertical slice: a short action-platforming dungeon in which lighting, combat, inventory and traversal are parts of one progression rather than separate demonstrations.
...
Nothing in the engine recognizes this level specially. Its rules are the ordinary editable `Assets/Scripts/Showcase.ts` behaviour embedded in `examples/prototypes/showcase/showcase.forge.json`; its scene uses standard Forge entities and components.
