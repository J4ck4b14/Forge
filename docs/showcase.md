...
Blackward is Forge's complete vertical slice: a three-room action-platforming dungeon in which darkness, combat, inventory and traversal form one continuous run.
...
Nothing in the engine recognizes this dungeon specially. Its rules are the ordinary editable `Assets/Scripts/Showcase.ts` behaviour embedded in `examples/prototypes/showcase/showcase.forge.json`; all three scenes use standard Forge entities and components. The general-purpose `ctx.session` service shares structured-cloned state between scene instances and is cleared when Play ends.
