...
Box, circle and capsule colliders are explicit components and may coexist as compounds. Collider-only entities receive fixed bodies. Circle/capsule colliders require uniform scale, and all physics rejects zero scale and shear with an entity-specific error. Use a static root for physics hierarchies. Continuous collision detection is exposed. Contact events identify Forge GUIDs and distinguish sensors; raycasts return Forge GUIDs, normal, point and distance, excluding sensors by default. A debug toggle renders Rapier line geometry through the camera.
