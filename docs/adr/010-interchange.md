...
Forge-native features remain free to exceed target capabilities. Arbitrary TypeScript is manual unless a specific translator exists. Generated target files must be separated from creator-owned files and preserve Forge GUID metadata. Round-trip target edits, undocumented native serialization, and silent omission are outside this architecture.
