...
This hotfix responds to the first clean Windows certification run of Forge 0.9.

- Renames the invalid `forge.shadowCaster` component identifier to the registry-compliant `forge.shadow-caster`.
...
- Removes the `python3` dependency from ZIP export tests. The replacement TypeScript reader validates Forge's store-only ZIP structure, CRC32 values, UTF-8 names, central-directory offsets/counts and byte preservation on every supported development OS.
- Adds a regression test for legacy shadow-caster canonicalization.
- Updates Vite from `7.3.1` to `7.3.2`, the patched 7.x release for CVE-2026-39363. This matters to Forge because the optional LAN/mobile workflow intentionally exposes the Vite development server to the local network.
