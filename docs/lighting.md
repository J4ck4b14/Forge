...
Add **Light 2D** to an entity. Forge supports ambient, point, spot and rectangular area lights. Color and intensity apply to every type. Point lights use range and falloff; spots add inner/outer full cone angles in degrees; area lights use width and height plus range outside the rectangle. A spot faces local +X, so rotate its Transform to aim it. Light position and direction inherit the hierarchy; range/size are world units and do not inherit Transform scale.
...
This is a useful foundation for stealth, but Forge does not prescribe detection AI: combine illumination with distance, facing/FOV, line-of-sight, sound and game-specific thresholds in project scripts.
