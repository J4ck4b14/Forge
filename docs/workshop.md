...
Open the URL printed by the terminal and choose a game. The launcher serves the three bundled production builds. Stop the server with Ctrl+C when finished. To author projects, run `npm run dev`, then use **Import project** to open a project's `.forge.json` file.
...
The new examples target a desktop keyboard and an approximately 800 by 500 game area. Use a browser window large enough to show it. The fighter requires two people at the same keyboard. No opponent AI or online play is implied.
...
2. Import one completed project from `examples/prototypes/shooter/shooter.forge.json`, `platformer/platformer.forge.json` or `fighter/fighter.forge.json`. These paths are relative to `examples/prototypes` after the first example.
...
4. Use **Save locally** for the browser's project store and **Export backup** for a portable `.forge.json` backup. Local storage belongs to that browser origin. Export a backup before changing origins/devices or clearing browser data.
...
| Item                              | Purpose                                        |
| --------------------------------- | ---------------------------------------------- |
| game name followed by .forge.json | Complete editable project with embedded assets |
...
Forge uses a centered 2D coordinate system. Positive X goes right; positive Y goes down. Positions and sprite dimensions are in world units. With Camera 2D zoom 1, one world unit occupies approximately one CSS pixel. The Inspector exposes Transform as a, b, c, d, x and y. Change x and y for position; keep a=1, b=0, c=0, d=1 for an unrotated unit scale. Use the viewport Rotate tool for visual rotation.
...
Create Hitbox 1 and Hitbox 2 at (0, 0), size 48 by 12, blank texture, lit false and visible false. Tint them #f3ac66 and #82baff respectively. These rectangles are visible active-phase indicators. They are not Forge physics colliders, and their dimensions are not the hit-rule parameters.
...
2. Serve that directory over HTTP. From the Forge project root, the included helper can serve an arbitrary export directory: `npm run preview:game -- my-game 4180`.
...
4. Upload the whole directory to your chosen static host. Relative paths support subdirectory hosting. Repeat the same checks at the deployed URL. No Forge editor, external CDN or runtime TypeScript compiler is required by the game.

A reproducible command-line export uses:

```sh
npm run build:player
npm run export:game -- examples/prototypes/shooter/shooter.forge.json my-game
