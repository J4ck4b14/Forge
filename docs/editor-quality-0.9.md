# Forge 0.9 — Editor Quality

Forge 0.9 deliberately freezes broad engine expansion and concentrates on authoring reliability, visual feedback and the cost model of systems that already exist.
...
Forge does not hard-code a stealth ruleset. It supplies composable primitives:
...
- `ctx.*` completion and searchable Forge API reference;
...
Forge continues to ship the three distinct Forge-authored prototypes used as practical regression targets: platformer, shooter and local fighter. The Lantern Shadow & Stealth Lab is an additional focused lighting/perception regression project.
