...
Project Browser creates, opens, renames, duplicates and deletes scenes, and sets the startup scene. Project name edits are undoable. **Save locally**/Ctrl+S writes to IndexedDB under the current browser origin. **Open local** lists those explicit saves with their last-update time. **Export backup** downloads the complete project as a portable `.forge.json`, and **Import project** validates and reopens it. A first-run notice explains that clearing site data, private browsing, changing device or changing site origin can make browser-local saves unavailable; portable exports are the cross-device/offline backup path. The optional **Account** dialog can also save/open projects through a reachable Forge account server; linked projects sync on normal Save and cloud writes use revision-conflict checks so an older device cannot silently overwrite a newer copy. See [account continuity](account-sync.md).
...
Mobile support means the authoring UI is usable on a modern touch browser; it does not imply that every large-project workflow is equally comfortable on a phone. For the zero-backend public alpha, Export backup → move the `.forge.json` file → Import project is the supported cross-device path. Optional account continuity remains available when a Forge sync server is configured.
