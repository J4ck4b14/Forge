# Forge 0.8 — lighting, authoring and continuity polish
...
Import `examples/lighting-shadow-demo/lighting-shadow-demo.forge.json` for a compact dungeon-style reference scene.
...
Forge remains local-first: IndexedDB saves and JSON import/export do not require an account. An optional reference account server adds cross-browser/device project continuity with authenticated per-user projects and revision-conflict protection.
...
```bash
FORGE_HOST=0.0.0.0 npm run dev:account
```
...
The later Forge 0.9.2 certification completed a clean dependency install, formatting, strict typecheck, lint/package-boundary checks, all 106 automated tests and both production builds on Windows. The public-alpha workflow repeats the same gates before GitHub Pages deployment.
