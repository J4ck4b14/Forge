...
Forge uses ordinary Git commits as its published development history. `git log` reads those commits and is what hosting services such as GitHub display. A reflog is different: it is local recovery metadata that records movements of references such as `HEAD` and branches.
