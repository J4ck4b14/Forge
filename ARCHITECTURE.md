...
| ---------------------- | ----------------------- | -------------------------------------------------------------------------- |
| `@forge/core`          | No third-party packages | Identity, components, world/hierarchy, affine math, events                 |
| `@forge/serialization` | Core, Zod 4.3.6         | Scene/project schemas, validation, deterministic serialization, migrations |
| `@forge/runtime`       | Core                    | Time and ordered system lifecycle                                          |
...
Project JSON is data only; there is no script evaluation, compilation or module loading. Future script execution needs an explicit origin/isolation threat model. No `eval`, generated authoring layer or AI product integration exists.
