...
import { exportUnity } from '@forge/interchange/unity';
...
it('generates a deterministic Unity project driven by supported Editor APIs', () => {
  ...
  model.change('Build Unity fixture', () => {
    ...
    model.world.add(entity, 'forge.sprite', {
      texture: image,
      secondaryTexture: '',
      blend: 1,
      lit: true,
    ...
    model.world.add(entity, 'forge.rigidbody');
    model.world.add(entity, 'forge.circle-collider');
  });
  ...
  const interchange = lowerProject(model.project),
    ...
    importer = decoded(files.get('Assets/Forge/Editor/ForgeImporter.cs')!),
    runtime = decoded(
      files.get('Assets/Forge/Runtime/ForgeGraphBehaviour.cs')!,
    );
  ...
  expect(
    decoded(files.get('Assets/Forge/Runtime/ForgeCoordinates.cs')!),
  ).toContain('UnitsPerPixel = 0.01f');
  expect(files.get(`Assets/Generated/Forge/Assets/${image}.png`)).toEqual(
    Uint8Array.of(1, 2, 3),
  );
