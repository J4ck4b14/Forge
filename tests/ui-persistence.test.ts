...
import { createRegistry, World } from '@forge/core';
import { SignalService } from '@forge/runtime';
import {
  RuntimeUiSystem,
  UiButton,
  UiLayout,
...
describe('runtime UI', () => {
  it('renders accessible hierarchy controls and routes interaction through signals', () => {
    ...
    ui.start();
    expect(host.querySelector('.forge-game-ui')?.textContent).toContain(
      'Score: 10',
    );
    ...
    ui.stop();
    expect(host.querySelector('.forge-game-ui')).toBeNull();
  });
});
