...
import { ScriptSystem, type ScriptModule } from '@forge/scripting';
import { Physics2D } from '@forge/physics2d/rapier';
import { InputService } from '@forge/input';
import { AnimationSystem } from '@forge/animation';
import {
  Light2D,
  ParticleSystem,
  SpriteRenderer,
...
import { UiText } from '@forge/ui';
import { Engine, TweenService } from '@forge/runtime';
import { SessionStateService } from '@forge/persistence';
async function game(
  name: string,
  sceneName?: string,
  ...
  model.load(
    JSON.parse(
      await readFile(`examples/prototypes/${name}/${name}.forge.json`, 'utf8'),
    ),
  );
  if (sceneName) {
...
it('shooter destroys six prefab drones with pooled swept projectiles and restarts a won game', async () => {
  ...
  try {
    expect([...g.model.world.query('forge.prefab')]).toHaveLength(6);
    g.key('Space', true);
