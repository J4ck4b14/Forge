...
/** Parse Forge's deliberately store-only ZIP format without any platform tool. */
function readStoredZip(data: Uint8Array): Map<string, Uint8Array> {
  ...
  while (
    ...
    if (compression !== 0 || compressedSize !== uncompressedSize)
      throw new Error('Forge export must use stored ZIP entries');
...
it('exports linked JS modules, a startup scene and a dependency report without shipping TS source', async () => {
  ...
  const project = JSON.parse(
    new TextDecoder().decode(
      files.find((f) => f.path === 'project.forge.json')!.data,
    ),
  );
  ...
  const dir = await mkdtemp(join(tmpdir(), 'forge-export-'));
  try {
