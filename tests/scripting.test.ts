...
import fixture from '../examples/physics-playground/Playground.forge.json';
...
it('loads compiled modules and executes two independent project mechanics without engine edits', async () => {
  ...
  try {
    ...
    const sprite = editor.world.components(pulse.id).get('forge.sprite') as {
      opacity: number;
    };
    ...
    expect(
      (
        editor.world.components(player.id).get('forge.sprite') as {
          tint: string;
        }
      ).tint,
    ).toBe('#8bffb3');
