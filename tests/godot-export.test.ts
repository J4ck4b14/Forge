...
it('generates a deterministic Godot 4 project with scenes, assets and graphs', () => {
  ...
  model.change('Build Godot fixture', () => {
    ...
    model.world.add(player, 'forge.sprite', {
      texture: image,
      secondaryTexture: '',
      blend: 1,
      lit: true,
    ...
    model.world.add(player, 'forge.camera');
    model.world.add(player, 'forge.rigidbody');
    model.world.add(player, 'forge.box-collider', {
      offsetX: 0,
      offsetY: 2,
      sensor: false,
      friction: 0.5,
  ...
  expect(scene).toContain('ForgeGraphRuntime.gd');
  expect(scene).toContain(`metadata/forge_id = "${child}"`);
  expect(files.get(`Generated/Forge/Assets/${image}.png`)).toEqual(
    Uint8Array.of(0),
  );
  expect(
    decoded(files.get('Generated/Forge/portability-report.json')!),
  ).toContain('fully-portable');
  expect(
    decoded(files.get('Generated/Forge/Scripts/ForgeGraphRuntime.gd')!),
