...
import { Tags, World, compose, decompose, guid } from '@forge/core';
...
import { RuntimePrefabs } from '@forge/player';
...
it('indexes tags and provides component, closest and radius queries', () => {
  ...
  expect(model.world.withComponents('forge.transform', Tags.type)).toHaveLength(
    2,
  );
});
...
it('runs the complete 0.10 behaviour-service acceptance workflow', async () => {
  ...
  model.project.assets.push({
    ...
    kind: 'text',
    mime: 'application/x-forge-prefab',
