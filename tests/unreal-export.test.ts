...
import { exportUnreal } from '@forge/interchange/unreal';
...
it('generates a deterministic UE5 C++ importer bundle without binary assets', () => {
  ...
  model.change('Build Unreal fixture', () => {
    ...
    model.world.add(entity, 'forge.box-collider');
  });
  ...
  const interchange = lowerProject(model.project),
    ...
    importerPath = [...files.keys()].find((path) =>
      path.endsWith('ForgeImporterModule.cpp'),
    )!,
    graphPath = [...files.keys()].find((path) =>
      path.endsWith('ForgeGraphComponent.cpp'),
    )!,
  ...
  expect(files.get(`ForgeSource/Assets/${image}.png`)).toEqual(
    Uint8Array.of(4, 5, 6),
  );
