...
import { instantiatePrefab, PREFAB_MIME } from '@forge/prefabs';
import { validateScene } from '@forge/serialization';
import type { RuntimePrefabService } from '@forge/scripting';
  ...
  instantiate(
    ...
    for (const entity of entities) {
      ...
      for (const [type, value] of Object.entries(entity.components))
        if (type === 'forge.transform') this.world.set(id, type, value);
