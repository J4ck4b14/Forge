...
import type { ScriptModule, ScriptFields } from '@forge/scripting';
import { runtimeRegistry } from './registry';
import { GameSession } from './session';
import { createPersistentServices } from '@forge/persistence';
import { frameDeltaSeconds } from '@forge/runtime';
...
const progress = (stage: string) =>
  window.dispatchEvent(
    new CustomEvent('forge:loading', { detail: { phase: 'loading', stage } }),
  );
function failure(error: unknown): void {
  failed = true;
  window.dispatchEvent(
    new CustomEvent('forge:loading', {
...
window.addEventListener('forge:startup-timeout', (event) =>
