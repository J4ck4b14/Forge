import type { ProjectData } from '@forge/serialization';
...
export class AccountSync {
  ...
  constructor() {
    try {
      this.endpoint = localStorage.getItem('forge.sync.endpoint') || '/api';
      this.token = sessionStorage.getItem('forge.sync.token') ?? '';
      const user = JSON.parse(sessionStorage.getItem('forge.sync.user') ?? 'null') as
        | { id: string; email: string }
        | null;
      if (user) this.userValue = user;
      const revisions = JSON.parse(localStorage.getItem('forge.sync.revisions') ?? '{}') as Record<string, number>;
  ...
  setEndpoint(value: string): void {
    ...
    try {
      localStorage.setItem('forge.sync.endpoint', next);
    } catch {
  ...
  private persist(): void {
    try {
      if (this.token) sessionStorage.setItem('forge.sync.token', this.token);
      else sessionStorage.removeItem('forge.sync.token');
      if (this.userValue) sessionStorage.setItem('forge.sync.user', JSON.stringify(this.userValue));
      else sessionStorage.removeItem('forge.sync.user');
      localStorage.setItem('forge.sync.revisions', JSON.stringify(Object.fromEntries(this.revisions)));
    } catch {
