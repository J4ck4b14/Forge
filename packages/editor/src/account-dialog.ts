  ...
  const render = () => {
    ...
    if (!sync.signedIn) {
      const email = input('Email', ''),
        ...
        status = node('p', 'settings-note', 'Cloud sync is optional and is not required for the GitHub Pages public alpha. Configure a reachable Forge sync server only if you want account-backed cross-device continuity.');
      ...
      const act = async (kind: 'signin' | 'signup') => {
        try {
          ...
          report(`${kind === 'signup' ? 'Created' : 'Signed into'} Forge account ${sync.user!.email}`);
          render();
        } catch (error) {
          fail(error);
        }
