...
import { scriptFields } from '@forge/scripting/compiler';
import {
  Behaviours,
  graphBehaviour,
  scriptBehaviour,
...
export { NoteComponent } from '@forge/player';
  ...
  applyRuntimeComponent(
    ...
    this.changeRuntimeAuthoring('Apply runtime component value', () => {
      ...
      if (type === TransformComponent.type) {
        const body = this.world.components(numeric).get('forge.rigidbody') as
          | { mode?: string }
          | undefined;
  ...
  paste(scene: SceneData): void {
    this.change('Paste entities', () => {
      ...
      for (const e of source.entities) {
        ...
        if (copy.parent === null) {
          const local = (
            copy.components['forge.transform'] as { local: number[] }
          ).local;
          local[4]! += 24;
          local[5]! += 24;
        }
