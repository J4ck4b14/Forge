...
import { runtimeRegistry } from '@forge/player';
import {
...
export function projectBuildFiles(input: ProjectData): BuildFile[] {
  ...
  files.push(
    { path: 'project.forge.json', data: encode(deterministicJSON(project)) },
    ...
    },
    {
      path: 'DEPLOY.txt',
      data: encode(
        'Upload this entire folder to static hosting (HTTPS recommended). Open index.html over HTTP(S), not file://. All paths are relative, so subdirectory hosting works. No Node, Forge editor, build service, or CDN is required by the player. Click Start for audio/input. See BUILD-REPORT.json for included content.\n',
      ),
    },
  );
  return files;
...
export async function previewBuild(
  ...
  try {
    ...
    const cache = await caches.open('forge-game-preview-v1');
    ...
    const prefix = new URL(
      `./__forge_preview__/${crypto.randomUUID()}/`,
      location.href,
    );
    for (const file of files) {
