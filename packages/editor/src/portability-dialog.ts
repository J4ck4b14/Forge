...
export function showPortability(model: EditorModel): void {
  ...
  dialog.append(
    ...
    node(
      'p',
      '',
      'Forge remains the source of truth. Reports classify every lowered item before one-way target export.',
    ),
  );
    ...
    if (report.target === 'godot')
      section.append(
        button('Export Godot project', async () => {
          const { exportGodot } = await import('@forge/interchange/godot');
          downloadArchive(
    ...
    if (report.target === 'unity')
      section.append(
        button('Export Unity project', async () => {
          const { exportUnity } = await import('@forge/interchange/unity');
          downloadArchive(
    ...
    if (report.target === 'unreal')
      section.append(
        button('Export Unreal project', async () => {
          const { exportUnreal } = await import('@forge/interchange/unreal');
          downloadArchive(
  ...
  actions.append(
    button('Download Interchange IR', () =>
      download(
        `${model.project.name.replace(/[^a-z0-9_-]/gi, '_')}.forge-ir.json`,
