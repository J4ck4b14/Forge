import { guid } from '@forge/core';
import {
  compileScript,
  compileProjectScripts,
  scriptDiagnostics,
} from '@forge/scripting/compiler';
import { ScriptBehaviour } from '@forge/scripting';
...
const templates = {
  Behaviour: `import type { ScriptContext } from '@forge/scripting';
...
`,
  Trigger: `import type { ScriptContext } from '@forge/scripting';
import type { ContactEvent } from '@forge/physics2d/rapier';
...
`,
  'Character Controller': `import type { ScriptContext } from '@forge/scripting';
...
`,
  Interaction: `import type { ScriptContext } from '@forge/scripting';
...
`,
  'UI Controller': `import type { ScriptContext } from '@forge/scripting';
...
export default class UIController {
  ...
  update(ctx: ScriptContext) {
    ...
    const sprite = ctx.get<SpriteData>('forge.sprite', this.indicator);
    if (sprite) ctx.set('forge.sprite', { ...sprite, visible: ctx.illumination() >= this.threshold }, this.indicator);
  }
}
`,
  'Stealth Guard': `import type { ScriptContext } from '@forge/scripting';
...
const draftPrefix = 'forge.scriptDraft.v2.';
...
export function showScripts(
  ...
  apiPanel.append(node('h3', '', 'Forge scripting API'), apiSearch.row, apiResults);
  renderApi();
