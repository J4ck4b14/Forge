...
import { LIGHTING_CHANNELS, type LightingChannel } from '@forge/renderer';
...
function exportBackup(): void {
  ...
  anchor.href = url;
  anchor.download =
    model.project.name.replace(/[^a-zA-Z0-9_-]/g, '_') + '.forge.json';
  anchor.click();
...
menu.append(
  ...
  button('Export backup', () => run(exportBackup), 'Download a portable .forge.json backup'),
  button('Import project', () => fileInput.click(), 'Open a portable .forge.json project'),
);
const fileInput = node('input');
fileInput.type = 'file';
fileInput.accept = '.forge.json,.json,application/json';
...
function renderHierarchy(): void {
  ...
  while (pending.length) {
    ...
    row.ondragstart = (e) => {
      ...
      e.dataTransfer?.setData('application/x-forge-entity', entity.guid);
    };
    row.ondragover = (e) => e.preventDefault();
    row.ondrop = (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (e.dataTransfer?.types.includes('application/x-forge-entity'))
...
refresh();
log(
  'Forge editor ready. Save locally stores projects in this browser; Export backup creates a portable .forge.json file.',
);
...
const STORAGE_NOTICE_KEY = 'forge.storage-notice.v1';
function showStorageNotice(): void {
  ...
  const dialog = node('dialog', 'storage-notice'),
    heading = node('h2', '', 'Where Forge saves your work'),
  ...
  dialog.append(
    ...
    node(
      'p',
      '',
      'Browser storage is convenient, not a portable backup. Clearing site data, using private browsing, changing device, or moving Forge to a different site can make local saves unavailable.',
    ),
    node(
      'p',
      'storage-notice-important',
      'Use Export backup regularly. The downloaded .forge.json can be kept anywhere and reopened later with Import project.',
    ),
  );
