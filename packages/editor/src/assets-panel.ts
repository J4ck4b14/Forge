...
import { CLIP_MIME, CONTROLLER_MIME } from '@forge/animation';
import { PREFAB_MIME } from '@forge/prefabs';
...
import { SpriteRenderer, renderList } from '@forge/renderer';
import { compileProjectScripts } from '@forge/scripting/compiler';
...
import { TILESET_MIME } from '@forge/tilemap';
...
export class AssetsPanel {
  ...
  render(): void {
    ...
    for (const path of direct) {
      ...
      b.ondrop = (e) => {
        e.preventDefault();
        const id = e.dataTransfer?.getData('application/x-forge-asset');
    ...
    for (const asset of [...this.model.project.assets]
      ...
      b.ondragstart = (e) =>
        e.dataTransfer?.setData('application/x-forge-asset', asset.id);
