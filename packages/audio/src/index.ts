...
import type { System } from '@forge/runtime';
...
export class AudioSystem implements System {
  ...
  private spatial(
    ...
    const source = this.world.find(id),
      cameras = this.world
        .withComponent('forge.camera')
        .filter((camera) => this.world.isActive(camera))
        .sort((a, b) => {
          const av = this.world.components(a).get('forge.camera') as
              | { priority?: number }
              | undefined,
            bv = this.world.components(b).get('forge.camera') as
              | { priority?: number }
              | undefined;
