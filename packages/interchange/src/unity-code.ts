...
namespace Forge.Generated
{
    ...
    public sealed class ForgeIdentity : MonoBehaviour
    {
        ...
        {
            foreach (ForgeIdentity identity in Object.FindObjectsOfType<ForgeIdentity>(true))
                if (identity.forgeId == id) return identity.gameObject;
            return null;
        }

        public void ForgeAnimationEvent(string payload)
        {
            gameObject.SendMessage("OnForgeAnimationEvent", payload, SendMessageOptions.DontRequireReceiver);
        }
    }
}
`;
...
namespace Forge.Generated
{
    public sealed class ForgeGraphBehaviour : MonoBehaviour
    {
        public TextAsset graphAsset;
        public InputActionAsset inputActions;
        public string forgeEntityId = "";
        ...
        {
            if (++steps > 1024) throw new InvalidOperationException("Forge graph exceeded 1024 flow steps");
        ...
        void Execute(JObject node)
        {
            ...
            else if (type == "transform.setPosition")
            {
                ...
                if (target != null) target.transform.position = new Vector3(ForgeCoordinates.X(value.x), ForgeCoordinates.Y(value.y), target.transform.position.z);
                Flow(node, "out");
            }
            else if (type == "physics.setVelocity")
            {
                ...
                if (body != null) body.velocity = new Vector2(ForgeCoordinates.X(value.x), ForgeCoordinates.Y(value.y));
                Flow(node, "out");
            }
        ...
        JToken Value(JObject node, string port)
        {
            ...
            if (type == "entity.self") return new JValue(forgeEntityId);
            if (type == "transform.position")
            {
                GameObject target = Entity(Input(node, "entity"));
                Vector3 value = target != null ? target.transform.position : Vector3.zero;
                return new JArray(ForgeCoordinates.PixelsX(value.x), ForgeCoordinates.PixelsY(value.y));
            }
            if (type == "input.axis")
            {
        ...
        {
            string id = token == null || token.Type == JTokenType.Null || string.IsNullOrEmpty((string)token) ? forgeEntityId : (string)token;
            return ForgeIdentity.Find(id);
        }
...
namespace Forge.Editor
{
    ...
    {
        const string Root = "Assets/Generated/Forge";
        const string IrPath = Root + "/Data/forge-ir.json";
        ...
        static ForgeImporter() { EditorApplication.delayCall += ImportIfChanged; }

        [MenuItem("Tools/Forge/Reimport")]
        public static void Import()
        {
            ...
            AssetDatabase.Refresh();
            StoreHash();
            Debug.Log("Forge import complete: " + scenePaths.Count + " scene(s). Review " + Root + "/Data/portability-report.json");
        }
        ...
        static string HashKey() { return "Forge.Interchange." + Application.dataPath; }
        ...
        static string BuildScene(JObject source)
        {
            ...
            foreach (JObject entity in source["entities"] as JArray)
            {
                ...
                var identity = gameObject.AddComponent<ForgeIdentity>();
                identity.forgeId = (string)entity["id"];
                identity.componentJson = entity["components"].ToString(Formatting.None);
                JObject tags = Component(entity, "forge.tags");
                ...
                entities[identity.forgeId] = gameObject;
            }
            foreach (JObject entity in source["entities"] as JArray)
            {
        ...
        static void ApplyTransform(Transform target, JArray matrix)
        {
            ...
            target.localPosition = new Vector3(ForgeCoordinates.X(F(matrix[4])), ForgeCoordinates.Y(F(matrix[5])), 0f);
            target.localEulerAngles = new Vector3(0f, 0f, ForgeCoordinates.Degrees(angle));
            target.localScale = new Vector3(scaleX, scaleY, 1f);
        }

        static void ApplyComponents(GameObject gameObject, JObject entity)
        {
            JObject bodyData = Component(entity, "forge.rigidbody");
            Rigidbody2D body = null;
            if (bodyData != null) body = AddBody(gameObject, bodyData);
            else if (Component(entity, "forge.character-body") != null)
            {
            ...
            foreach (JObject item in entity["components"] as JArray)
            {
                ...
                if (type == "forge.sprite") AddSprite(gameObject, data);
                else if (type == "forge.camera") AddCamera(gameObject, data);
                else if (type == "forge.audio-source") AddAudio(gameObject, data);
                else if (type == "forge.animator") AddAnimator(gameObject, data);
                else if (type == "forge.box-collider") AddBox(gameObject, data);
                else if (type == "forge.circle-collider") AddCircle(gameObject, data);
                else if (type == "forge.capsule-collider") AddCapsule(gameObject, data);
                else if (type == "forge.behaviours") AddGraphs(gameObject, entity, data);
                else if (type == "forge.particle-emitter") AddParticles(gameObject, data);
            }
        }
        ...
        static Rigidbody2D AddBody(GameObject gameObject, JObject data)
        {
            ...
            body.velocity = new Vector2(ForgeCoordinates.X(F(data, "velocityX")), ForgeCoordinates.Y(F(data, "velocityY")));
            body.angularVelocity = ForgeCoordinates.Degrees(F(data, "angularVelocity"));
            return body;
        }
        ...
        static void AddSprite(GameObject gameObject, JObject data)
        {
            ...
            renderer.size = new Vector2(ForgeCoordinates.Length(F(data, "width", 64f)), ForgeCoordinates.Length(F(data, "height", 64f)));
        ...
        static void AddAudio(GameObject gameObject, JObject data)
        {
            ...
            source.minDistance = ForgeCoordinates.Length(F(data, "minDistance", 64f));
            source.maxDistance = ForgeCoordinates.Length(F(data, "maxDistance", 800f));
        ...
        static void AddBox(GameObject gameObject, JObject data)
        {
            ...
            collider.size = new Vector2(ForgeCoordinates.Length(F(data, "width", 64f)), ForgeCoordinates.Length(F(data, "height", 64f)));
        }
        ...
        static void AddCircle(GameObject gameObject, JObject data)
        {
            ...
            CommonCollider(collider, data);
            collider.radius = ForgeCoordinates.Length(F(data, "radius", 32f));
        }
        ...
        static void AddCapsule(GameObject gameObject, JObject data)
        {
            ...
            collider.size = new Vector2(ForgeCoordinates.Length(radius * 2f), ForgeCoordinates.Length((radius + half) * 2f));
        }

        static void CommonCollider(Collider2D collider, JObject data)
        {
            collider.offset = new Vector2(ForgeCoordinates.X(F(data, "offsetX")), ForgeCoordinates.Y(F(data, "offsetY")));
            ...
            var material = new PhysicsMaterial2D(collider.gameObject.name + " Forge Material");
            material.friction = F(data, "friction", .5f);
            material.bounciness = F(data, "restitution");
            string path = Root + "/Physics/" + collider.GetComponent<ForgeIdentity>().forgeId + "-" + collider.GetType().Name + ".physicsMaterial2D";
        ...
        static void AddGraphs(GameObject gameObject, JObject entity, JObject data)
        {
            ...
            foreach (JToken order in data["order"] as JArray)
            {
                ...
                var graph = gameObject.AddComponent<ForgeGraphBehaviour>();
                graph.enabled = B(item, "enabled", true);
                graph.forgeEntityId = (string)entity["id"];
        ...
        static void AddParticles(GameObject gameObject, JObject data)
        {
            ...
            main.startSpeed = new ParticleSystem.MinMaxCurve(ForgeCoordinates.Length(F(data, "speedMin", 30f)), ForgeCoordinates.Length(F(data, "speedMax", 90f)));
            main.startSize = ForgeCoordinates.Length(F(data, "startSize", 12f));
            ...
            force.enabled = true;
            force.x = ForgeCoordinates.X(F(data, "gravityX"));
            force.y = ForgeCoordinates.Y(F(data, "gravityY", 120f));
        ...
        {
            string path = Root + "/Input/ForgeInput.inputactions";
            AssetDatabase.DeleteAsset(path);
        ...
        static void PrepareAnimations()
        {
            ...
            foreach (JObject asset in AssetsById.Values)
                if ((string)asset["mime"] == "application/x-forge-animation") BuildClip(asset);
            AssetDatabase.SaveAssets();
            foreach (JObject asset in AssetsById.Values)
                if ((string)asset["mime"] == "application/x-forge-animator") BuildController(asset);
        }
        ...
        static void BuildClip(JObject asset)
        {
            ...
            clip.name = S(source, "name", "Forge Clip");
            clip.frameRate = 60f;
            ...
            foreach (JObject item in source["events"] as JArray)
                events.Add(new AnimationEvent { time = F(item, "time"), functionName = "ForgeAnimationEvent", stringParameter = item["payload"] == null ? S(item, "name") : S(item, "name") + ":" + item["payload"].ToString(Formatting.None) });
        ...
        static void ConfigurePhysics()
        {
            ...
            Physics2D.gravity = new Vector2(ForgeCoordinates.X(F(physics, "gravityX")), ForgeCoordinates.Y(F(physics, "gravityY", 980f)));
        ...
        static void PrepareMedia()
        {
            foreach (JObject asset in AssetsById.Values)
            {
                ...
                if (importer != null && (importer.textureType != TextureImporterType.Sprite || importer.spritePixelsPerUnit != ForgeCoordinates.PixelsPerUnit))
                {
                    importer.textureType = TextureImporterType.Sprite;
                    importer.spriteImportMode = SpriteImportMode.Single;
                    importer.spritePixelsPerUnit = ForgeCoordinates.PixelsPerUnit;
        ...
        static JObject ResolveImage(JObject asset)
        {
            ...
            if ((string)asset["mime"] != "application/x-forge-sprite-region") return null;
        ...
        static string Safe(string value) { string result = new string(value.Select(character => char.IsLetterOrDigit(character) || character == '_' || character == '-' ? character : '_').ToArray()).Trim('_'); return string.IsNullOrEmpty(result) ? "Forge" : result; }
