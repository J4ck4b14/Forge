...
public class ForgeImporter : ModuleRules
{
    ...
    {
        PCHUsage = PCHUsageMode.UseExplicitOrSharedPCHs;
        PublicDependencyModuleNames.AddRange(new[] { "Core", "CoreUObject", "Engine", "ForgeRuntime" });
...
#include "ForgeIdentityComponent.generated.h"

UCLASS(ClassGroup=(Forge), meta=(BlueprintSpawnableComponent))
class FORGERUNTIME_API UForgeIdentityComponent : public UActorComponent
{
    GENERATED_BODY()
public:
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Forge") FGuid ForgeId;
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Forge") TArray<FName> ForgeTags;
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Forge", meta=(MultiLine=true)) FString ComponentJson;

    static AActor* Find(const UWorld* World, const FGuid& Id);
};
`;

export const UNREAL_IDENTITY_CPP = `#include "ForgeIdentityComponent.h"
#include "EngineUtils.h"

AActor* UForgeIdentityComponent::Find(const UWorld* World, const FGuid& Id)
{
    if (!World || !Id.IsValid()) return nullptr;
    for (TActorIterator<AActor> It(World); It; ++It)
        if (const UForgeIdentityComponent* Identity = It->FindComponentByClass<UForgeIdentityComponent>())
            if (Identity->ForgeId == Id) return *It;
    return nullptr;
}
`;
...
#include "ForgeGraphComponent.generated.h"

UCLASS(ClassGroup=(Forge), meta=(BlueprintSpawnableComponent))
class FORGERUNTIME_API UForgeGraphComponent : public UActorComponent
{
    GENERATED_BODY()
public:
    UForgeGraphComponent();
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Forge") FString GraphFile;
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Forge") FGuid ForgeEntityId;
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Forge", meta=(MultiLine=true)) FString OverridesJson = TEXT("{}");
...
export const UNREAL_GRAPH_CPP = `#include "ForgeGraphComponent.h"
#include "ForgeCoordinates.h"
#include "ForgeIdentityComponent.h"
...
namespace
{
    ...
    FString Safe(const FString& Source)
    {
        ...
        return Result.IsEmpty() ? TEXT("Forge") : Result;
    }
}

UForgeGraphComponent::UForgeGraphComponent()
{
    PrimaryComponentTick.bCanEverTick = true;
}

void UForgeGraphComponent::BeginPlay()
{
    Super::BeginPlay();
    FString Source;
    if (!FFileHelper::LoadFileToString(Source, *GraphFile)) { UE_LOG(LogTemp, Error, TEXT("Forge graph missing: %s"), *GraphFile); return; }
    const TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(Source);
    if (!FJsonSerializer::Deserialize(Reader, Graph) || !Graph.IsValid()) { UE_LOG(LogTemp, Error, TEXT("Forge graph invalid: %s"), *GraphFile); return; }
...
void UForgeGraphComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UForgeGraphComponent::Fire(const FString& Type)
{
...
void UForgeGraphComponent::Flow(const TSharedPtr<FJsonObject>& Source, const FString& Port)
{
    if (++Steps > 1024) { UE_LOG(LogTemp, Error, TEXT("Forge graph exceeded 1024 flow steps")); return; }
...
void UForgeGraphComponent::Execute(const TSharedPtr<FJsonObject>& Current)
{
    ...
    {
        if (AActor* Target = Entity(Input(Current, TEXT("entity")))) { const FVector2D Position = Vector(Input(Current, TEXT("position"))); Target->SetActorLocation(ForgeCoordinates::Position(Position.X, Position.Y)); }
        Flow(Current, TEXT("out"));
    }
    else if (Type == TEXT("physics.setVelocity"))
    {
        if (AActor* Target = Entity(Input(Current, TEXT("entity")))) if (UPrimitiveComponent* Body = Target->FindComponentByClass<UPrimitiveComponent>()) { const FVector2D Velocity = Vector(Input(Current, TEXT("velocity"))); Body->SetPhysicsLinearVelocity(ForgeCoordinates::Velocity(Velocity.X, Velocity.Y)); }
        Flow(Current, TEXT("out"));
    }
    else if (Type == TEXT("debug.log")) { const TSharedPtr<FJsonValue> Message = Input(Current, TEXT("message")); UE_LOG(LogTemp, Log, TEXT("Forge: %s"), Message.IsValid() ? *Message->AsString() : *(*Properties)->GetStringField(TEXT("message"))); Flow(Current, TEXT("out")); }
}

TSharedPtr<FJsonValue> UForgeGraphComponent::Value(const TSharedPtr<FJsonObject>& Current, const FString& Port)
{
    ...
    if (Type == TEXT("entity.self")) return MakeShared<FJsonValueString>(ForgeEntityId.ToString(EGuidFormats::DigitsWithHyphensLower));
    if (Type == TEXT("transform.position"))
    {
        ...
        TArray<TSharedPtr<FJsonValue>> Result{ MakeShared<FJsonValueNumber>(ForgeCoordinates::PixelsX(Position.X)), MakeShared<FJsonValueNumber>(ForgeCoordinates::PixelsY(Position.Z)) }; return MakeShared<FJsonValueArray>(Result);
    }
    if (Type == TEXT("input.axis") || Type == TEXT("input.vector2"))
    {
        const FString ActionName = (*Properties)->GetStringField(TEXT("action"));
        const UInputAction* Action = LoadObject<UInputAction>(nullptr, *(TEXT("/Game/Generated/Forge/Input/IA_") + Safe(ActionName) + TEXT(".IA_") + Safe(ActionName)));
    ...
    if (Type == TEXT("physics.velocity"))
    {
        ...
        TArray<TSharedPtr<FJsonValue>> Result{ MakeShared<FJsonValueNumber>(ForgeCoordinates::PixelsX(Velocity.X)), MakeShared<FJsonValueNumber>(ForgeCoordinates::PixelsY(Velocity.Z)) }; return MakeShared<FJsonValueArray>(Result);
    }
    return MakeShared<FJsonValueNull>();
}

TSharedPtr<FJsonValue> UForgeGraphComponent::Input(const TSharedPtr<FJsonObject>& Current, const FString& Port)
{
    const TArray<TSharedPtr<FJsonValue>>* Connections = nullptr;
...
TSharedPtr<FJsonObject> UForgeGraphComponent::Node(const FString& Id) const
{
...
AActor* UForgeGraphComponent::Entity(const TSharedPtr<FJsonValue>& Token) const
{
    FGuid Id = ForgeEntityId; FString Text;
    if (Token.IsValid() && Token->TryGetString(Text) && !Text.IsEmpty()) FGuid::Parse(Text, Id);
    return UForgeIdentityComponent::Find(GetWorld(), Id);
}
`;

export const UNREAL_RUNTIME_MODULE = `#include "Modules/ModuleManager.h"

IMPLEMENT_MODULE(FDefaultModuleImpl, ForgeRuntime)
`;
...
export const UNREAL_EDITOR_CPP = `#include "ForgeImporterModule.h"
#include "ForgeCoordinates.h"
#include "ForgeGraphComponent.h"
#include "ForgeIdentityComponent.h"
...
namespace ForgeImport
{
    const FString Generated = TEXT("/Game/Generated/Forge");
    ...
    FString Safe(const FString& Source) { FString Result; for (TCHAR Character : Source) Result.AppendChar(FChar::IsAlnum(Character) || Character == '_' ? Character : '_'); return Result.IsEmpty() ? TEXT("Forge") : Result; }
    ...
    void ImportMedia()
    {
        ...
        for (const TPair<FString, TSharedPtr<FJsonObject>>& Pair : Assets)
        {
            ...
            UAssetImportTask* Task = NewObject<UAssetImportTask>(); Task->Filename = FPaths::ProjectDir() / TEXT("ForgeSource/Assets") / (Pair.Key + Extension); Task->DestinationPath = Generated + TEXT("/Assets"); Task->DestinationName = Pair.Key; Task->bAutomated = true; Task->bReplaceExisting = true; Task->bSave = true;
    ...
    UPaperSprite* Sprite(const FString& Id)
    {
        ...
        FString SourceId = Id;
        if (String(*Asset, TEXT("mime")) == TEXT("application/x-forge-sprite-region")) { TSharedPtr<FJsonObject> Region; FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(String(*Asset, TEXT("data"))), Region); if (Region.IsValid()) SourceId = String(Region, TEXT("source")); }
    ...
    UPaperFlipbook* Flipbook(const FString& Id)
    {
        ...
        TSharedPtr<FJsonObject>* Asset = Assets.Find(Id); if (!Asset || String(*Asset, TEXT("mime")) != TEXT("application/x-forge-animation")) return nullptr;
    ...
    void BuildInput()
    {
        UInputMappingContext* Context = Cast<UInputMappingContext>(NewAsset(UInputMappingContext::StaticClass(), TEXT("Input"), TEXT("ForgeInput")));
    ...
    void AddComponents(AActor* Actor, const TSharedPtr<FJsonObject>& Entity)
    {
        ...
        const TSharedPtr<FJsonObject> Body = Component(Entity, TEXT("forge.rigidbody"));
        for (const TSharedPtr<FJsonValue>& Item : *Components)
        {
            ...
            else if (Type == TEXT("forge.behaviours"))
            {
                ...
                for (const TSharedPtr<FJsonValue>& OrderItem : *Order) { FString Key; if (!OrderItem->TryGetString(Key)) continue; const TSharedPtr<FJsonObject>* Behaviour = nullptr; if (!(*Items)->TryGetObjectField(Key, Behaviour) || String(*Behaviour, TEXT("kind")) != TEXT("graph")) continue; UForgeGraphComponent* Value = NewObject<UForgeGraphComponent>(Actor); Actor->AddInstanceComponent(Value); Value->ForgeEntityId = Guid(String(Entity, TEXT("id"))); Value->GraphFile = FPaths::ProjectDir() / TEXT("ForgeSource/Data/graphs") / (String(*Behaviour, TEXT("graph")) + TEXT(".json")); Value->OverridesJson = Json((*Behaviour)->GetObjectField(TEXT("values"))); Value->RegisterComponent(); }
            }
        }
    }
    ...
    void BuildScene(const TSharedPtr<FJsonObject>& Source)
    {
        ...
        for (const TSharedPtr<FJsonValue>& Item : *Entities)
        {
            ...
            UForgeIdentityComponent* Identity = NewObject<UForgeIdentityComponent>(Actor); Actor->AddInstanceComponent(Identity); Identity->ForgeId = Guid(String(*Entity, TEXT("id"))); Identity->ComponentJson = Json((*Entity)->GetArrayField(TEXT("components"))); const TSharedPtr<FJsonObject> Tags = Component(*Entity, TEXT("forge.tags")); const TArray<TSharedPtr<FJsonValue>>* TagValues = nullptr; if (Tags.IsValid() && Tags->TryGetArrayField(TEXT("tags"), TagValues)) for (const TSharedPtr<FJsonValue>& TagValue : *TagValues) { FString Tag; if (TagValue->TryGetString(Tag)) { Identity->ForgeTags.Add(FName(*Tag)); Actor->Tags.Add(FName(*Tag)); } } Identity->RegisterComponent(); Actors.Add(String(*Entity, TEXT("id")), Actor);
        }
        for (const TSharedPtr<FJsonValue>& Item : *Entities)
        {
            ...
            Actor->SetActorRelativeLocation(ForgeCoordinates::Position(Matrix[4]->AsNumber(), Matrix[5]->AsNumber())); Actor->SetActorRelativeRotation(FRotator(ForgeCoordinates::Degrees(Rotation), 0.0, 0.0)); Actor->SetActorRelativeScale3D(FVector(ScaleX, 1.0, ScaleY)); Actor->SetActorHiddenInGame(!Boolean(*Entity, TEXT("enabled"), true)); Actor->SetActorEnableCollision(Boolean(*Entity, TEXT("enabled"), true)); AddComponents(Actor, *Entity);
        }
        Save(World);
    }
    ...
    bool Run()
    {
        FString Source; const FString Path = FPaths::ProjectDir() / TEXT("ForgeSource/Data/forge-ir.json"); if (!FFileHelper::LoadFileToString(Source, *Path)) return false;
...
void FForgeImporterModule::StartupModule() { UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(this, &FForgeImporterModule::RegisterMenus)); ImportIfChanged(); }
void FForgeImporterModule::ShutdownModule() { if (UToolMenus::IsToolMenuUIEnabled()) UToolMenus::UnRegisterStartupCallback(this); }
void FForgeImporterModule::RegisterMenus() { FToolMenuOwnerScoped Owner(this); UToolMenu* Menu = UToolMenus::Get()->ExtendMenu(TEXT("LevelEditor.MainMenu.Tools")); FToolMenuSection& Section = Menu->FindOrAddSection(TEXT("Forge")); Section.AddMenuEntry(TEXT("ForgeReimport"), FText::FromString(TEXT("Forge Reimport")), FText::FromString(TEXT("Rebuild generated content from Forge Interchange IR")), FSlateIcon(), FUIAction(FExecuteAction::CreateRaw(this, &FForgeImporterModule::Import))); }
void FForgeImporterModule::ImportIfChanged() { FString Source; if (!FFileHelper::LoadFileToString(Source, *(FPaths::ProjectDir() / TEXT("ForgeSource/Data/export-manifest.json")))) return; TSharedPtr<FJsonObject> Manifest; if (!FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(Source), Manifest)) return; const FString Hash = ForgeImport::String(Manifest, TEXT("sourceHash")); FString Previous; GConfig->GetString(TEXT("ForgeImport"), TEXT("SourceHash"), Previous, GEditorPerProjectIni); if (Hash != Previous) Import(); }
void FForgeImporterModule::Import() { if (ForgeImport::Run()) { FString Source; FFileHelper::LoadFileToString(Source, *(FPaths::ProjectDir() / TEXT("ForgeSource/Data/export-manifest.json"))); TSharedPtr<FJsonObject> Manifest; FJsonSerializer::Deserialize(TJsonReaderFactory<>::Create(Source), Manifest); GConfig->SetString(TEXT("ForgeImport"), TEXT("SourceHash"), *ForgeImport::String(Manifest, TEXT("sourceHash")), GEditorPerProjectIni); GConfig->Flush(false, GEditorPerProjectIni); UE_LOG(LogTemp, Display, TEXT("Forge import complete. Review ForgeSource/Data/portability-report.json")); } }
