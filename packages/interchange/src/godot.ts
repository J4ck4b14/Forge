...
function mainNodeType(entity: InterchangeEntity): string {
  if (component(entity, 'forge.character-body')) return 'CharacterBody2D';
  const body = component(entity, 'forge.rigidbody'),
...
function imageResource(
  ...
  if (asset.mime !== 'application/x-forge-sprite-region') return undefined;
...
function collisionMask(layer: number, interchange: ForgeInterchange): number {
  let mask = 0;
...
function colliderLines(
  ...
  for (const candidate of entity.components) {
    ...
    result.push(
      ...
      `metadata/forge_sensor = ${boolean(data.sensor)}`,
      `metadata/forge_one_way = ${boolean(data.oneWay)}`,
      `metadata/forge_friction = ${number(data.friction, 0.5)}`,
      `metadata/forge_restitution = ${number(data.restitution)}`,
      `metadata/forge_collision_layer = ${1 << layer}`,
      `metadata/forge_collision_mask = ${collisionMask(layer, interchange)}`,
    );
  }
  return result;
}
...
function sceneFile(
  ...
  const writer = new SceneWriter(),
    ...
    nodes: string[] = [
      ...
      `metadata/forge_id = ${godotString(scene.id)}`,
    ];
  for (const entity of ordered.entities) {
    const fullPath = ordered.paths.get(entity.id)!,
      ...
      body = component(entity, 'forge.rigidbody'),
      ...
      tags = component(entity, 'forge.tags'),
    ...
    nodes.push(
      ...
      `metadata/forge_id = ${godotString(entity.id)}`,
      `metadata/forge_components = ${godotString(json(entity.components))}`,
    );
    if (firstCollider) {
    ...
    const sprite = component(entity, 'forge.sprite');
    if (sprite) {
    ...
    const camera = component(entity, 'forge.camera');
    if (camera)
      nodes.push(
        '',
        ...
        'enabled = true',
        `metadata/forge_priority = ${number(camera.priority)}`,
        `metadata/forge_background = ${godotString(text(camera.background, '#101820'))}`,
      );
    const light = component(entity, 'forge.light');
    if (light && text(light.kind) !== 'ambient') {
      const texture = writer.externalResource(
        'Generated/Forge/Textures/radial-light.svg',
        'Texture2D',
      );
      nodes.push(
        '',
        ...
        `metadata/forge_kind = ${godotString(text(light.kind, 'point'))}`,
        `metadata/forge_falloff = ${number(light.falloff, 1)}`,
        `metadata/forge_inner_angle = ${number(light.innerAngle)}`,
        `metadata/forge_outer_angle = ${number(light.outerAngle, 360)}`,
      );
    }
    const audio = component(entity, 'forge.audio-source');
    if (audio) {
    ...
    const behaviours = component(entity, 'forge.behaviours'),
    ...
    for (const id of order) {
      ...
      const script = writer.externalResource(
        'Generated/Forge/Scripts/ForgeGraphRuntime.gd',
        'Script',
      );
      nodes.push(
        '',
        ...
        `graph_path = ${godotString(`res://Generated/Forge/Data/graphs/${graph.id}.json`)}`,
        `forge_entity_id = ${godotString(entity.id)}`,
...
function projectFile(interchange: ForgeInterchange): string {
  const startup =
    ...
    direction = gravity
      ...
      : [0, 1];
  return [
    '; Generated by Forge. Edit source in Forge and export again.',
    'config_version=5',
    '',
    '[application]',
    ...
    '',
    '[autoload]',
    `ForgeInput=${godotString('*res://Generated/Forge/Scripts/ForgeInput.gd')}`,
    '',
    '[audio]',
    'default_bus_layout="res://Generated/Forge/default_bus_layout.tres"',
    '',
    '[display]',
...
function mixerFile(interchange: ForgeInterchange): string {
...
function inputScript(interchange: ForgeInterchange): string {
  return `extends Node
...
static func _action(name: String) -> StringName: return StringName("forge_" + name.to_snake_case())
...
const GRAPH_RUNTIME = `class_name ForgeGraphRuntime
extends Node

@export_file("*.json") var graph_path: String
@export var forge_entity_id: String
...
\tif file == null:
\t\tpush_error("Forge graph missing: " + graph_path)
\t\treturn
...
\tif type == "entity.self": return forge_entity_id
\tif type == "transform.position":
\t\tvar target = _entity(_input(node, "entity"))
\t\treturn [target.position.x, target.position.y] if target else [0, 0]
\tif type == "input.axis": return ForgeInput.axis(str(node["properties"].get("action", "")))
\tif type == "input.vector2":
\t\tvar value = ForgeInput.vector(str(node["properties"].get("action", "")))
...
func _entity(id):
\tvar wanted = forge_entity_id if id == null or str(id).is_empty() else str(id)
\treturn _find_entity(get_tree().current_scene, wanted)

func _find_entity(root: Node, id: String):
\tif root.get_meta("forge_id", "") == id: return root
...
Generated for Godot 4.x by Forge ${interchange.source.engineVersion}. Open \`project.godot\` in the Godot editor. Generated content lives under \`Generated/Forge\`; keep target-side custom work outside that directory because re-export replaces it.

Forge remains the source of truth. This is a one-way export, not a round trip. Review \`Generated/Forge/portability-report.json\` before shipping.
...
export function exportGodot(interchange: ForgeInterchange): ExportFile[] {
  const manifest = createExportManifest(interchange, 'godot'),
    files: ExportFile[] = [
      ...
      {
        path: 'Generated/Forge/forge-ir.json',
        data: encodeText(serializeInterchange(interchange)),
      },
      {
        path: 'Generated/Forge/export-manifest.json',
        data: encodeText(json(manifest)),
      },
      {
        path: 'Generated/Forge/portability-report.json',
        data: encodeText(serializeReport(manifest.report)),
      },
      {
        path: 'Generated/Forge/Scripts/ForgeInput.gd',
        data: encodeText(inputScript(interchange)),
      },
      {
        path: 'Generated/Forge/Scripts/ForgeGraphRuntime.gd',
        data: encodeText(GRAPH_RUNTIME),
      },
      {
        path: 'Generated/Forge/default_bus_layout.tres',
        data: encodeText(mixerFile(interchange)),
      },
      {
        path: 'Generated/Forge/Textures/radial-light.svg',
  ...
  for (const asset of interchange.assets) {
    ...
    if (asset.mime === GRAPH_MIME)
      files.push({
        path: `Generated/Forge/Data/graphs/${asset.id}.json`,
