...
function normalizeLegacyComponentTypes(input: unknown): unknown {
  ...
  for (const entity of input.entities) {
    ...
    if (Object.hasOwn(record, 'forge.shadowCaster')) {
      if (!Object.hasOwn(record, 'forge.shadow-caster'))
        record['forge.shadow-caster'] = record['forge.shadowCaster'];
      delete record['forge.shadowCaster'];
    }
  }
  return input;
}
