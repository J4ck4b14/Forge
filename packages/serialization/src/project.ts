import { animationAssets, CONTROLLER_MIME } from '@forge/animation';
import { MixerSchema, defaultMixer } from '@forge/audio';
import {
  PREFAB_MIME,
  validatePrefab,
  validateLinks,
...
import { PhysicsSettingsSchema, defaultPhysics } from '@forge/physics2d';
import { InputMapSchema, defaultInput } from '@forge/input';
import { AssetSchema, AssetDatabase, assetReferences } from '@forge/assets';
import { z } from 'zod';
import { guid, type ComponentRegistry } from '@forge/core';
import {
  ...
  const migrateScene = (candidate: unknown): void => {
    ...
    for (const entity of entities) {
      ...
      if (legacy && typeof legacy === 'object' && !Array.isArray(legacy)) {
        ...
        delete record['forge.script'];
      }
      const link = record['forge.prefab'];
      if (
        link &&
        typeof link === 'object' &&
        !Array.isArray(link) &&
        ...
        for (const override of link.overrides)
          if (
            ...
            const index = override.path.indexOf('forge.script');
            if (index >= 0)
...
export function validateProject(
  ...
  for (let sceneIndex = 0; sceneIndex < project.scenes.length; sceneIndex++) {
    ...
    for (const entity of scene.entities) {
      const animator = entity.components['forge.animator'] as
        | { controller?: string }
        | undefined;
      if (animator?.controller && !entity.components['forge.sprite'])
    ...
    for (const entity of scene.entities)
      for (const [type, data] of Object.entries(entity.components)) {
        ...
        if (type === 'forge.audio-source' && data && typeof data === 'object') {
          if (
            'clip' in data &&
            data.clip &&
            !project.assets.some(
        ...
        if (
          (type === 'forge.sprite' || type === 'forge.behaviours') &&
          data &&
          typeof data === 'object'
        ) {
          ...
          for (const reference of references)
            if (reference.id) {
              ...
              if (
                asset &&
                ((type === 'forge.sprite' && asset.kind !== 'image') ||
                  (type === 'forge.behaviours' &&
  ...
  const documents = [...project.scenes, ...bases.values()],
    behaviourReferences = documents.flatMap((scene) =>
      scene.entities.flatMap((entity) => {
        const data = entity.components['forge.behaviours'];
        if (
          !data ||
          typeof data !== 'object' ||
          !('items' in data) ||
