# Forge flagship showcase and focused labs
...
| ---------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **The Luminous Vault** | `showcase/showcase.forge.json`     | Flagship vertical slice: lighting, melee/ranged combat, inventory, multiweapon switching, puzzle progression, platforming, enemy projectiles, UI, audio and camera feedback |
| Signal Patrol          | `shooter/shooter.forge.json`       | Focused top-down movement, projectile pooling and enemy lab                                                                                                                 |
| Lantern Steps          | `platformer/platformer.forge.json` | Focused platforming, collectible and checkpoint lab                                                                                                                         |
| Sparring Room          | `fighter/fighter.forge.json`       | Focused two-player local combat lab                                                                                                                                         |
...
Each source folder includes raw images, audio and scripts, and each `.forge.json` project embeds the same assets. Editing a raw file does not automatically update the embedded project; run `npm run examples:generate` only when intentionally regenerating the checked-in samples.

The workshop remains available as `docs/Forge-Prototype-Workshop.docx` and `docs/workshop.md` for step-by-step reconstruction of the focused labs. The flagship is the recommended first experience; the smaller projects exist to isolate mechanics for learning and debugging.
