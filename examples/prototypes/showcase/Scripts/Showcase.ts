...
export default class Showcase {
  ...
  private collectPickups(c: ScriptContext): void {
    const [x, y] = c.position(),
      near = (name: string, radius = 33) => {
        ...
        if (!id || !c.get<SpriteData>('forge.sprite', id)?.visible)
          return false;
  ...
  private setVisible(c: ScriptContext, id: string, visible: boolean): void {
    const current = c.get<SpriteData>('forge.sprite', id);
  ...
  private setLightById(
    ...
    const current = c.get<LightData>('forge.light', id);
    if (current) c.set('forge.light', { ...current, ...changes }, id);
  }
}
