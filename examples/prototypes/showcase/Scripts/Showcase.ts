...
import type { LightData, SpriteData } from '@forge/renderer';
...
/**
 * A single vertical slice deliberately exercises Forge systems together:
...
export default class Showcase {
  ...
  private collectPickups(c: ScriptContext): void {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      if (
        c.get<SpriteData>('forge.sprite', c.find(name)!)?.visible &&
        near(name)
      ) {
        this.ammo += 4;
        show(c, name, false);
    ...
    if (
      ...
      !this.hasKey &&
      c.get<SpriteData>('forge.sprite', c.find('Vault Key')!)?.visible &&
      near('Vault Key')
    ) {
      this.hasKey = true;
  ...
  private setLight(
    ...
    const id = c.find(name)!,
      current = c.get<LightData>('forge.light', id)!;
    c.set('forge.light', { ...current, ...changes }, id);
  }
