...
Forge's flagship vertical slice is a three-room action-platforming dungeon built entirely from ordinary project entities and one editable TypeScript behaviour.
...
Import `showcase.forge.json` into Forge to inspect every entity, light and script. The standalone launcher build is generated with `npm run build:prototypes`.
